...
{
    public class ProductItemUpload
    {
        ...
        { 
            ...
            return MssqlHelper.GetDataSet("[dbo].[SP_WEB_Site_PopupPage_ProductItemUpload_List_R]", Params, CommandType.StoredProcedure);
        }
        ...
        public string SetExecludeBulkUpload(DataTable dt,string userid,string tablename)
        {
            ...
            try
            {
                ...
                Params[0] = new SqlParameter("@USERID", userid);
                Params[1] = new SqlParameter("@chk", "PRODUCTGET");
                ...
                if (strMsg == "OK")
                {
                    ...
                    ds = MssqlHelper.GetDataSet("[dbo].[SP_WEB_Site_PopupPage_ProductItemUpload_Validate_S]", Params, CommandType.StoredProcedure);
                    if (ds != null)
                    {
                        if (ds.Tables[0].Rows.Count > 0)
                        {
        ...
        public string SetExecludeConfirm(string userid)
        {
            ...
            DataSet ds = MssqlHelper.GetDataSet("[dbo].[SP_WEB_Site_PopupPage_ProductItemUpload_Confirm_S]", Params, CommandType.StoredProcedure);
            if (ds != null)
            {
                if (ds.Tables[0].Rows.Count > 0)
                {
