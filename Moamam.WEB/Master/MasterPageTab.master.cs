...
public partial class Master_MasterPageTab : System.Web.UI.MasterPage
{
    ...
    protected void rptSubmenu_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType == ListItemType.AlternatingItem || e.Item.ItemType == ListItemType.Item)
        {
            ...
            //hlnSubmenu.NavigateUrl = row["MENU_URL"].ToString();
            if (row["MENU_NM"].ToString() == "마스터코드관리" ||row["MENU_NM"].ToString() == "사용자관리" ||row["MENU_NM"].ToString() == "메뉴관리")
            { 
            } else {
                hlnSubmenu.NavigateUrl = "javascript:jsfn_addTopTab('" + row["MENU_NM"].ToString().Replace("'", "") + "','" + row["MENU_URL"].ToString().Replace("~", "") + "')";
            }
    ...
    private void MenuPath()
    {
        ...
        {
            lblpath.Text = ds.Tables[0].Rows[0]["MENU_GROUP_NM"].ToString();
            lblCurrentNoade.Text = ds.Tables[0].Rows[0]["MENU_NM"].ToString();
        }
    }
