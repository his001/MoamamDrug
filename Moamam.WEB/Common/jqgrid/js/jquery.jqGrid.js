...
(function ($) {
    ...
    $.fn.jqGrid = function (pin) {
		...
		formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata) {
		    ...
		    if (rowInd === 0) {
		        ...
		        if (celp && typeof celp === "string") {
                    ...
                    var stlCnt = celp.replace('-style', '-sti').indexOf("style");
		            if (acp.length === 2) {
		            ...
		            //TODO:ie8및 이하 버전에서 style설정시 적용안되는 문제 해결 DATE:2014.04.14 CREATOR:jin.chunhe
		            else if ($.jgrid.msie && $.jgrid.msiever() < 9 && acp.length === 1 && stlCnt > -1) {
		                acp[0] = $.trim(acp[0].replace('-sti', '-style').replace("=", ""));
		                if (acp[0].indexOf("'") === 0 || acp[0].indexOf('"') === 0) {
		                    acp[0] = acp[0].substring(1);
		                }
		                result += acp[0].replace(/'/gi, '"');
		            } else {
		                result += "\"";
		            }
		        }
		...
		addCell = function (rowId, cell, pos, irow, srvr, rdata) {
		    ...
		    if (v === undefined || v === null || v === "0.00") v = "";
		...
		setPager = function (pgid, tp) {
			...
			pginp = "",
			pgl = "<table cellspacing='0' cellpadding='0' border='0' style='table-layout:auto;height:40px;' class='ui-pg-table'><tbody><tr style='v-align:middle;'>",
		    ...
		    $("#" + $.jgrid.jqID(pgid))
			.append("<div id='" + pgcnt + "' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='min-width:350px;width:100%;table-layout:fixed;height:100%;' role='row'><tbody><tr><td id='" + lft + "' align='left'></td><td id='" + cent + "' align='center' style='white-space:pre;'></td><td id='" + rgt + "' align='right'></td></tr></tbody></table></div>")
		    ...
		    if (dir === "rtl") { pgl += str; }
		    if (ts.p.pginput === true) { pginp = "<td dir='" + dir + "' style='width:150px;padding-top:10px;'><table><tr><td style='text-align:center;'>" + $.jgrid.format(ts.p.pgtext || "", "</td><td style='text-align:right;'><input class='ui-pg-input' type='text' size='2' maxlength='7' value='0' role='textbox'/></td><td  style='text-align:left;'>", "<span id='sp_1_" + $.jgrid.jqID(pgid) + "'></span>") + "</td></tr></table></td>"; }
		    if (ts.p.pgbuttons === true) {
		...
		firstr = "<tr class='jqgfirstrow' role='row' style='height:auto'>";
            ...
            $("th", thr).each(function (j) {
                ...
                if (!(nm === 'cb' || nm === 'subgrid' || nm === 'rn' || nm.substr(0, 4) === 'chk_')) {
...
$.jgrid.extend({
	...
	setFrozenColumns : function () {
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				$t.grid.fbDiv = $('<div style="position:absolute;left:0px;top:'+(parseInt(top,10)+parseInt(hth,10) + 0)+'px;overflow-y:hidden" class="frozen-bdiv ui-jqgrid-bdiv"></div>');
				...
				$($t).bind('jqGridAfterGridComplete.setFrozenColumns', function () {
                    ...
                    //chu.yingjie add by 20150911 frozencolumn height
                    if(btbl.height()<$(this).height())
                    {
                    var f_rows = btbl.find("tr");
                    for(var i = 0;i<f_rows.length;i++)
                    {
                    $(f_rows[i]).height($($(this).find("tr")[i]).height());
                    }
	...
	setFrozenColumnsAllowEdit : function () {
		return this.each(function() {
			if ( !this.grid ) {return;}
			var $t = this, cm = $t.p.colModel,i=0, len = cm.length, maxfrozen = -1, frozen= false;
			// TODO treeGrid and grouping  Support
...
// Date Validation Javascript
	checkDate : function (format, date) {
		...
		for(i=0;i<format.length;i++){
		 ...
		 //TODO: MODIFYER:jin.chunhe, MODI_DATE:2014.06.23
         //y.m.d격식의 날짜 검증시 오류 수정
        // 'y' 를 'Y'로 수정
	...
	checkValues : function(val, valref, customobject, nam) {
		...
		if(edtrul) {
			...
			if(edtrul.minValue !== undefined && !isNaN(edtrul.minValue)) {
                ...
                //TODO MDFY:jin.chunhe MDATE:2014-10-29 DESC: 优化了多语言下的提示信息
                    msg = $.jgrid.edit.msg.minValue.replace('{0}', edtrul.minValue);
                    return [false, nm + ": " + msg, ""];
			...
			if(edtrul.maxValue !== undefined && !isNaN(edtrul.maxValue)) {
                ...
                //TODO MDFY:jin.chunhe MDATE:2014-10-29 DESC: 优化了多语言下的提示信息
                    msg = $.jgrid.edit.msg.maxValue.replace('{0}',edtrul.maxValue);
                    return [false, nm+": " + msg, ""];
			...
			if(edtrul.integer === true) {
				if( !(rqfield === false && $.jgrid.isEmpty(val)) ) {
                    ...
                    //TODO:MDFY:jin.chunhe MDATE:2014-10-29 DESC:val==-0 或023格式的数字也筛选掉
                    if(val.toString()==='-0'){ return [false,nm+": "+$.jgrid.edit.msg.integer,""]; }
                    filter = /^[1-9]\d*|0$/;
                    if(!filter.test(val.toString())) { return [false,nm+": "+$.jgrid.edit.msg.integer,""]; }
				}
			}
			if(edtrul.date === true) {
				if(!$.jgrid.isEmpty(val)) {//!(rqfield === false && $.jgrid.isEmpty(val))
				...
				if (v2 !== $t.p.savedRow[fr].v){
					...
					if(cv[0] === true) {
						...
						try {
                            ...
                            //TODO:MODIFIER:jin.chunhe MODI-DATE:2014-10-29 DESC:change messagebox to wf
                            $.wf.alert(v+" "+cv[1]);
...
/*
* Copyright 2014 jin.chunhe MIT ,GPT Lisence
*/
(function ($) {
    $.jgrid.extend({
        headAggData: function (action, data, format) {
            var nm, success = false, res = {}, title;
            function isEmpty(obj) {
                var i;
                for (i in obj) {
            ...
            this.each(function () {
                var t = this, vl;
			    if(!t.grid || !t.p.headaggrow) {return false;}
                        
                if (action === "set") { if (isEmpty(data)) { return false; } }
                success = true;
                if ($("tr.aggrow", t.grid.hDiv).size() === 0) {
                    var cntCol = $(".ui-jqgrid-btable tr.jqgfirstrow", t.grid.bDiv).children().size();
                    var agrow = $("<tr>").addClass('aggrow');
                    for (var i = 0; i < cntCol; i++) {
                        agrow.append($("<th>"));
                    }
                    $("thead", t.grid.hDiv).append(agrow);
                }
                else {
                    $("tr.aggrow th", t.grid.hDiv).empty();
                }
                $(this.p.colModel).each(function (i) {
                    nm = this.name;
                    var halign = this.align;
                    if (action === "set") {
                        if (data[nm] !== undefined) {
                            vl = format ? t.formatter("", data[nm], i, data, 'edit') : data[nm];
                            title = this.title ? { "title": $.jgrid.stripHtml(vl)} : {};
                            $("tr.aggrow th:eq("+i+")",t.grid.hDiv).html(vl).attr(title);
                            success = true;
                        }
                        if(halign !==undefined){
                            $("tr.aggrow th:eq(" + i + ")", t.grid.hDiv).addClass("text-" + halign);
                        }
                    } else if (action === "get") {
                        res[nm] = $("tr.footrow td:eq(" + i + ")", t.grid.sDiv).html();
                    }
                ...
                if (t.p.frozenColumns) {
                    var hth = $(".ui-jqgrid-htable", "#gview_" + $.jgrid.jqID(t.p.id)).height();
                    var tarow = $("tr.aggrow", t.grid.hDiv).clone(true);
                    var maxfrozen = t.grid.fbDiv.find(".ui-jqgrid-btable tr").first().size();
                    if(!t.p.rownumbers){maxfrozen = maxfrozen-1;}
                    tarow.find("th:gt("+maxfrozen+")").remove();
                    t.grid.fhDiv.height(hth).find(".ui-jqgrid-htable tr.aggrow").remove()
                    .end().find(".ui-jqgrid-htable").append(tarow);
                    t.grid.fbDiv.css("top", parseInt(hth, 10) + 1);
                }
            });
            return action === "get" ? res : success;
        }
